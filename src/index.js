...
app.post('/user/:user_id/activate-dataset', async (req, res) => {
    ...
    try {
        await userDatasetHandler.activate(userId, datasetId);
        res.status(200).send();
    } catch (e) {
...
app.post('/user/:user_id/deactivate-dataset', async (req, res) => {
    ...
    const datasetId = req.body.datasetId;
    try {
        await userDatasetHandler.deactivate(userId, datasetId);
        res.status(200).send();
    } catch (e) {
...
app.post('/user/:user_id/active', async (req, res) => {
    const userId = req.params.user_id;
    try {
        const activeDatasets = await userDatasetHandler.getActive(userId, datasetId);
        res.status(200).json(activeDatasets);
    } catch (e) {
